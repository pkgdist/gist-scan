    ...
    await new Command()
      ...
      .action(async (opts) => { 
        ...
        for (const gist of gists) {
          await self.getFileContents(gist);
          if (self.matchesSearch(gist, searchTerms, regexTerms)) matches.push(gist);
        }
        ...
        for (const gist of matches) {
          ...
          console.log(`Description: ${this.highlightMatches(gist.files.key.content?.toString() || "", searchTerms, regexTerms)}`);
          for (const file of Object.values<{ filename: string; raw_url: string; content?: string }>(gist.files)) {
